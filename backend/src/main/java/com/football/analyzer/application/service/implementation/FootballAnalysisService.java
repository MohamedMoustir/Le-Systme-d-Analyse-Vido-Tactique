...
public class FootballAnalysisService {
    ...
    public void startAnalysis(String videoId) {
        ...
        try {
            String inputPath = new File("uploads/" + video.getUrlFichier()).getAbsolutePath();
            String outputPath = inputPath.replace(".mp4", "_annotated.mp4");
            ...
            if (exitCode != 0) {
                log.error("❌ Python failed with code: {}", exitCode);
