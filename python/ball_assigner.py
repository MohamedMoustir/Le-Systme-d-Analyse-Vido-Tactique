...
from config import EASYOCR_AVAILABLE, EASYOCR_AVAILABLE, PADDLEOCR_AVAILABLE
from annotator import VideoAnnotator
from ocr_engine import JerseyNumberRecognizer
from team_assigner import TeamAssigner
from utils import get_foot_position, output_json, get_center_of_bbox,measure_distance
class PlayerBallAssigner:
    def __init__(self, max_distance=70):
        self.max_player_ball_distance = max_distance

    def assign_ball_to_player(self, players_bboxes, players_ids, ball_bbox):
        ...
        for i, bbox in enumerate(players_bboxes):
            ...
            distance_left = measure_distance((bbox[0], bbox[3]), ball_position)
            distance_right = measure_distance((bbox[2], bbox[3]), ball_position)
...
#     def __init__(self, model_path, device='cpu', teams_config_path=None, enable_ocr=True, enable_annotation=True, use_world_model=False):
#         self.model = YOLO(model_path)
#         self.use_world_model = use_world_model
#         self.use_soccer_model = False  # Specialized soccer model (SportsVision-YOLO)
#         self.device = device
#         self.ball_model = None  # Secondary model for ball detection
#         self.use_hybrid_mode = False  # Nano for players + Soccer for ball
#         self.team_ball_control = {1: 0, 2: 0}
#         self.initialized = False
#         self.goal_cooldown = 0  # Compteur bach ntsnaw bin les goals
#         self.last_event = None
#         self.use_half = device == 'cuda'
#         # --- 1. LOAD GOAL MODEL (Zid hadchi) ---
...
#         # Enable Half Precision for Goal Model too
#         if self.use_half and self.goal_model:
#             try:
#                 self.goal_model.model.half()
#             except: pass
#         # Enable half precision (FP16) for GPU - much faster inference
#         self.use_half = device == 'cuda'
#         if self.use_half:
#             self.model.to(device)
#             # Pre-warm the model with half precision
#             try:
#                 import torch
...
#         # Detect model type based on filename or classes
...
#             # SportsVision-YOLO or similar specialized soccer model
#             # Classes: {0: 'player', 1: 'ball', 2: 'logo'}
...
#             # YOLOv8 Nano - try to load soccer model for ball detection (HYBRID MODE)
#             soccer_model_path = os.path.join(os.path.dirname(model_path), 'yolov8m_soccer.pt')
...
#         # Load teams config for team colors
#         teams_config = None
...
#         # Jersey number recognition (PaddleOCR preferred, EasyOCR fallback)
...
#         self.ocr_frame_interval = 3  # Run OCR every N frames to save performance

#         # Video annotation - pass display colors from team_assigner
...
#         # Ball tracking - keep last known position for interpolation
#         self.last_ball_bbox = None
#         self.ball_not_found_count = 0
#         self.max_ball_interpolation_frames = 15  # Max frames to keep showing last ball position

#         # Ball motion tracking for prediction
#         self.ball_positions_history = []  # List of (frame_num, center_x, center_y)
#         self.max_ball_history = 5  # Keep last 5 positions for velocity estimation
...
#         # Use smaller slices (400x400) for better small ball detection
#         # This allows YOLO to see the ball at higher effective resolution
#         slice_size = 400
#         overlap = 100  # 25% overlap
...
#                 try:
#                     # Detect on slice with low confidence for ball
#                     # Using imgsz=640 on a 400x400 slice gives good resolution
...
#         # Generate strategic slice positions (400x400 slices)
#         slice_size = 400
#         slices = []

#         # Cover the field in a grid pattern (3x3 = 9 slices for typical 1280x720)
#         # This is faster than full SAHI but covers most of the field
...
#         """Analyze a single frame and return JSON-serializable results

#         Args:
#             frame: The video frame to analyze
#             frame_num: Frame number
#             fast_mode: If True, use faster but less accurate settings
#             ultra_fast_mode: If True, skip SAHI and rely on motion interpolation (for streaming)
#         """

#         # Use half precision on GPU for faster inference
#         half_param = self.use_half if hasattr(self, 'use_half') else False

#         # Run detection - configuration based on model type
#         # FAST settings for streaming
#         if self.use_soccer_model:
#             # Specialized soccer model: fast settings
#             conf_threshold = 0.15
#             imgsz = 480  # Smaller = faster
...
#             try:
#                 # detect goal net
#                 g_res = self.goal_model.predict(frame, conf=0.20, verbose=False, device=self.device, half=half_param)[0]
#                 if len(g_res.boxes) > 0:
#                     # Take largest box (closest net)
...
#         # B. Check Intersection (Ball inside Net)
#         if ball_bbox and goal_bbox_detected and self.goal_cooldown == 0:
...
#                 SystemError.stderr.write(f"⚽ GOAL DETECTED via AI at frame {frame_num}\n")

#         if self.goal_cooldown > 0:
#             self.goal_cooldown -= 1
#             if self.goal_cooldown > 100: event_type = "GOAL"
...
#     def stream_mjpeg(self, video_path, skip_frames=1, target_width=1280, use_sahi=False, output_video_path=None):
...
#         # Video writer disabled - stream only mode (no file saving)
#         video_writer = None
...
#                 try:
#                     # Annotate frame with its corresponding AI result
#                     if result and self.annotator:
#                         annotated_frame = self.annotator.annotate_frame(frame, result)
#                     else:
...
#                 # Send notification that annotated video is ready
...
#                 sys.stderr.flush()
#         else:
#             sys.stderr.write(f"Warning: No video writer was active\n")
#             sys.stderr.flush()
...
#             "final_possession": self.possession.get_team_possession() if self.possession else {}
#         }
